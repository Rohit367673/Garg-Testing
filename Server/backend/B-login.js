...
mongoose
  .connect("mongodb+srv://rohit673367:Rohit367673@cluster0.ixnakmg.mongodb.net/signup")
  ...
  const razorpay = new Razorpay({
    key_id: process.env.RAZORPAY_KEY_ID || "rzp_test_cqRGqNXYhc4b3x", // Hardcoded value for testing
    key_secret: process.env.RAZORPAY_KEY_SECRET// Hardcoded value for testing
  });
  app.post("/create-order", async (req, res) => {
    try {
      ...
      const processedCartItems = cartItems.map(item => {
        // Use item.productId if available; otherwise, fallback to item.id.
      ...
      const newOrder = new OrderModel({
        ...
        cartId: cartId && mongoose.Types.ObjectId.isValid(cartId)
  ...
  app.post("/verify-payment", async (req, res) => {
    try {
      ...
      // Generate a signature using Razorpay key_secret
      ...
      await OrderModel.findByIdAndUpdate(dbOrderId, {
        paymentStatus: "Paid",
        orderStatus: "Processing", // Change order status as per your logic
  ...
  app.get("/orders", async (req, res) => {
    try {
      const orders = await OrderModel.find(); // Optionally, you can add .populate() if needed
  ...
  app.post("/addCart", async (req, res) => {
    ...
    try {
      ...
      if (!cart) {
        ...
        // Check if the same product (with matching options) exists in cartItems
        ...
        if (indexFound !== -1) {
          ...
          // Otherwise, push the new item into the cartItems array
          cart.cartItems.push({
            productId: new mongoose.Types.ObjectId(productId),
            productName,
            imgsrc,
        ...
        // Update the subtotal based on all items
      ...
      // Save the updated (or new) cart document
      await cart.save();
  ...
  app.get("/products", async (req, res) => {
    try {
      ...
      // Convert _id to id in response and include images
...
app.post('/admin/login', (req, res) => {
  console.log("Admin login attempt:", req.body); // Debug log to verify incoming request
  const { Email, Pass } = req.body; // Client sends Email and Pass
...
const adminPassword = process.env.ADMIN_PASSWORD || "rohit367673"
  ...
  // Optional: Log the credentials (only during development, remove in production)
  ...
  if (Email === adminEmail && Pass === adminPassword) {
    // Create a token that includes the role info (admin)
